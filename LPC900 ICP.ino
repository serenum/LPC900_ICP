...
void setup() {
  ...
  /* GPIO for ICP */
  pinMode(ICP_VCC, OUTPUT);
  pinMode(ICP_RESET, OUTPUT);
  pinMode(ICP_PCL, OUTPUT);
  pinMode(ICP_PDA, OUTPUT);
  digitalWrite(ICP_VCC, HIGH);

  /* ISP UART */
  Serial.begin(9600);

  delay(500);   // delay 500 msec to stabilize before entering ICP mode
...
void enter_icp(void) {
  ...
  /* Power off target */
  digitalWrite(ICP_RESET, LOW);
  digitalWrite(ICP_VCC, HIGH);
  delay(10);
  /* Power on target with reset held low */
  digitalWrite(ICP_VCC, LOW);
  delay(5);
  for (pulses = 0; pulses < 7; pulses++)  // pulse reset 7 times
  {
    digitalWrite(ICP_RESET, HIGH);
    delayMicroseconds(30);  // wait about 15 usec
    digitalWrite(ICP_RESET, LOW);
    delayMicroseconds(30);  // wait about 15 usec
  }
  digitalWrite(ICP_RESET, HIGH);  // hold reset high
}
...
void shift_out(char data_byte) {
  ...
  {
    digitalWrite(ICP_PCL, LOW);  // hold clock line low
    if (temp_byte & 0x01)        // check if LSB is set
    {
      digitalWrite(ICP_PDA, HIGH);  // set data-line high
    } else {
      digitalWrite(ICP_PDA, LOW);  // set data-line low
    }
    temp_byte = (data_byte >>= 1);  // shift databyte right one, put in temp
    digitalWrite(ICP_PCL, HIGH);    // clock  databit
  }
  digitalWrite(ICP_PDA, HIGH);  // set dataline high after transfer
  digitalWrite(ICP_PCL, HIGH);  // hold clockline high after transfer
}
...
char shift_in(void) {
  char data_byte = 0;
  char shift_bit = 0;
  digitalWrite(ICP_PCL, LOW);
  pinMode(ICP_PDA, INPUT);
  for (shift_bit = 0; shift_bit < 8; shift_bit++)  // shift in 8 bits
  {
    digitalWrite(ICP_PCL, HIGH);  // clock out databit
    if (digitalRead(ICP_PDA))     // check if PDA is set
    {
  ...
  digitalWrite(ICP_PCL, LOW);   // set clockline low after transfer
  digitalWrite(ICP_PDA, HIGH);  // hold dataline high
  pinMode(ICP_PDA, OUTPUT);
...
unsigned char echo() {
  ...
  ch = Serial.read();
  Serial.flush();             // wait until previous transmission is complete
  Serial.print((char)ch);     // send character
